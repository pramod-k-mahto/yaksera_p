...
export default function EditPortfolio() {
  ...
  const goTo   = (sid) => { setError(""); setSuccess(""); setActiveSection(sid); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Only append gallery if user picked new files
      galleryFiles.forEach((f) => fd.append("gallery", f));

      await updatePortfolio(id, fd);
  ...
  const completeness = Math.round(
    [form.title, form.slug, form.category, form.description, imagePreview]
  ...
  if (fetching) {
    return (
      <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", minHeight: "100vh", background: "#f8f7f4", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#0d1f4e" strokeWidth="2.5" style={{ animation: "spin .8s linear infinite" }}>
    ...
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", minHeight: "100vh", background: "#f8f7f4", padding: "2rem 1rem" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .save-btn{flex:1;padding:14px;background:#0d1f4e;color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:600;font-family:inherit;cursor:pointer;transition:background .18s,transform .12s;letter-spacing:.01em}
        .save-btn:hover:not(:disabled){background:#162f6b}
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "2rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            <button
              ...
              style={{ width: 36, height: 36, background: "#f3f4f6", border: "1.5px solid #e5e3df", borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="2.2"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>
            </button>
            <div>
              <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 26, fontWeight: 700, color: "#0d1f4e", margin: 0, lineHeight: 1 }}>Edit Portfolio</h1>
              <p style={{ fontSize: 13, color: "#9ca3af", margin: "2px 0 0" }}>Update project details and media</p>
            </div>
          </div>
            ...
            <div style={{ padding: "28px 28px 0" }}>
              ...
              <div className={`section-panel${activeSection === "basics" ? " active" : ""}`}>
                <div style={{ display: "grid", gap: 18 }}>
                  <div>
                    ...
                    <input name="title" placeholder="e.g. E-commerce Dashboard" value={form.title} onChange={handleChange} className="ap-input" onKeyDown={(e) => e.key === "Enter" && e.preventDefault()} />
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    <div>
                      <p className="label-text">Slug</p>
                      <input name="slug" placeholder="auto-generated" value={form.slug} onChange={handleChange} className="ap-input" style={{ fontFamily: "monospace", fontSize: 13 }} onKeyDown={(e) => e.key === "Enter" && e.preventDefault()} />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea name="description" placeholder="Describe the project..." value={form.description} onChange={handleChange} className="ap-input" />
                  </div>
                  <div>
                    ...
                    <input name="tags" placeholder="React, Node.js, AWS..." value={form.tags} onChange={handleChange} className="ap-input" onKeyDown={(e) => e.key === "Enter" && e.preventDefault()} />
                    {form.tags && (
              ...
              <div className={`section-panel${activeSection === "media" ? " active" : ""}`}>
                <div style={{ display: "grid", gap: 20 }}>
                  <div>
                    <p className="label-text">Cover Image <span style={{ color: "#bbb", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>— leave empty to keep current</span></p>
                    <div className="file-input-wrapper">
                      <div className="file-drop-zone">
                        ...
                        {imageFile
                          ...
                          : <><p style={{ fontSize: 13, margin: 0 }}>Click or drag to replace cover image</p><p style={{ fontSize: 11, margin: "4px 0 0", color: "#bbb" }}>Max 5MB</p></>
                        }
                      </div>
                  ...
                  <div>
                    <p className="label-text">Gallery <span style={{ color: "#bbb", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>— new uploads append to existing</span></p>

                    {/* existing gallery */}
                    {existingGallery.length > 0 && (
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                        {existingGallery.map((url, i) => (
                          <img key={i} src={url} alt={`existing-${i}`} className="gallery-thumb" />
                        ))}
                      </div>
                    )}
                    ...
                    <div className="file-input-wrapper">
                      <div className="file-drop-zone">
                        ...
                        {galleryFiles.length > 0
                          ? <p style={{ fontSize: 13, color: "#0d1f4e", fontWeight: 500, margin: 0 }}>✅ {galleryFiles.length} new file(s) selected</p>
                          : <><p style={{ fontSize: 13, margin: 0 }}>Click or drag to add more gallery images</p><p style={{ fontSize: 11, margin: "4px 0 0", color: "#bbb" }}>Select multiple — max 5</p></>
                        }
                      </div>
                    ...
                    {galleryFiles.length > 0 && (
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                        {galleryFiles.map((file, i) => (
                          <img key={i} src={URL.createObjectURL(file)} alt={`new-${i}`} className="gallery-thumb" />
                        ))}
                      </div>
                    )}
                  </div>
                ...
                <div style={{ display: "grid", gap: 18 }}>
                  ...
                  <div>
                    ...
                    <div style={{ position: "relative" }}>
                      <input name="projectUrl" placeholder="https://yourproject.com" value={form.projectUrl} onChange={handleChange} className="ap-input" style={{ paddingLeft: 40 }} onKeyDown={(e) => e.key === "Enter" && e.preventDefault()} />
            ...
            <div style={{ padding: "24px 28px 28px" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                {!isLast ? (
                  ...
                  <button type="button" disabled={loading} className="save-btn" onClick={handleSave}>
                    {loading
        ...
        <p style={{ textAlign: "center", fontSize: 12, color: "#c4c0b8", marginTop: 20 }}>
          Fields marked <span style={{ color: "#e8192c" }}>*</span> are required
        </p>
      </div>
    </div>
  );
