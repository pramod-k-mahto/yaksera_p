...
import { createJobVacancies } from "../services/jobVacancy";
...
const defaultForm = {
  ...
  applicationDeadline: "",
  featured: false,
  salary: { min: "", max: "", currency: "USD", isVisible: false },
  status: "open",
};
      ...
      {items.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-1">
          {items.map((item, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium bg-slate-800 border border-slate-700 text-slate-300"
            >
              {item}
              <button
                type="button"
                onClick={() => onChange(items.filter((_, j) => j !== i))}
                className="text-slate-500 hover:text-rose-400 transition ml-0.5 leading-none"
              >
                ×
              </button>
            </span>
...
function Toggle({ checked, onChange, label }) {
  return (
    <label className="flex items-center gap-3 cursor-pointer select-none w-fit">
      ...
      >
        <div
          className={`absolute top-[3px] w-[18px] h-[18px] rounded-full bg-white shadow transition-transform duration-200 ${checked ? "translate-x-[22px]" : "translate-x-[3px]"}`}
        />
      </div>
...
export default function AddVacancy() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await createJobVacancies(payload);
      setToast({ type: "success", msg: "Job vacancy published successfully!" });
      setForm(defaultForm);
      console.log(res);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#080b12] text-slate-100 font-sans pb-20">
      ...
      {toast && (
        <div
          className={`fixed top-5 right-5 z-50 flex items-center gap-2 px-5 py-3.5 rounded-xl shadow-2xl text-sm font-semibold transition-all
      ...
      <div className="bg-gradient-to-br from-slate-900 via-[#0d0f1a] to-[#080b12] border-b border-slate-800/60 px-6 py-12 mb-8">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-slate-500 text-sm">
            Fill in the details below to publish a new opening to your careers page.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto px-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" min="1" value={form.openings}
                onChange={(e) => set("openings", e.target.value)} className={inputCls} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.location} onChange={(e) => set("location", e.target.value)}
                placeholder="e.g. Kathmandu, Nepal" className={inputCls} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.education} onChange={(e) => set("education", e.target.value)}
                placeholder="e.g. Bachelor's in Computer Science" className={inputCls} />
            </div>
            <div>
              <Label>Application Deadline</Label>
              <input type="datetime-local" value={form.applicationDeadline}
        ...
        <SectionCard accent="text-amber-400" icon="📝" title="Job Description">
          <div>
            ...
            <textarea
              ...
              rows={5}
              placeholder="Describe the role, team culture, and what you're looking for..."
        ...
        <SectionCard accent="text-emerald-400" icon="📋" title="Role Details">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <TagInput label="Responsibilities" items={form.responsibilities}
              onChange={(v) => set("responsibilities", v)}
              placeholder="Add a responsibility…" />
            <TagInput label="Requirements" items={form.requirements}
              onChange={(v) => set("requirements", v)}
              placeholder="Add a requirement…" />
            <TagInput label="Benefits" items={form.benefits}
              onChange={(v) => set("benefits", v)}
              placeholder="Add a benefit…" />
            <TagInput label="Skills / Tags" items={form.tags}
        ...
        <SectionCard accent="text-pink-400" icon="💰" title="Compensation">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" min="0" value={form.salary.min}
                onChange={(e) => setSalary("min", e.target.value)}
                placeholder="e.g. 50000" className={inputCls} />
            </div>
            <div>
              <Label>Max Salary</Label>
              <input type="number" min="0" value={form.salary.max}
                onChange={(e) => setSalary("max", e.target.value)}
                placeholder="e.g. 80000" className={inputCls} />
            </div>
            <div>
              <Label>Currency</Label>
              <select value={form.salary.currency}
                onChange={(e) => setSalary("currency", e.target.value)} className={selectCls}>
          ...
          <Toggle
            ...
            onChange={() => setSalary("isVisible", !form.salary.isVisible)}
